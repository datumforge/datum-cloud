...
type Server struct {
	...
	// Datum contains the token for the datum server
	Datum Datum `json:"datum" koanf:"datum"`
}
...
type Datum struct {
	// Token is the token used to authenticate with the datum server
...
func Load(cfgFile *string) (*Config, error) {
	...
	// load env vars
	if err := k.Load(env.Provider("DATUMCLOUD_", ".", func(s string) string {
		return strings.ReplaceAll(strings.ToLower(
			strings.TrimPrefix(s, "DATUMCLOUD_")), "_", ".")
	}), nil); err != nil {
		panic(err)
	}
